    ...
    private static <T> List<Node> convetData2Node(List<T> datas)
        ...
        for (T t : datas) {
            ...
            for (Field f : declaredFields) {
                ...
                if (f.getAnnotation(TreeNodeLevel.class) != null) {
                    f.setAccessible(true);
                    level = f.getInt(t);
                }
                if (f.getAnnotation(TreeNodeOnline.class) != null) {
                    f.setAccessible(true);
                    online = (String) f.get(t);
                }
                if (f.getAnnotation(TreeNodeAlias.class) != null) {
                    f.setAccessible(true);
                    alias = (String) f.get(t);
                }
                if (id != -1 && pId != -1 && label != null) {
                    break;
                }
            }
            node = new Node(id, pId, label, level, online, alias);
            if (3 == level) {
                if (online.equals("offline")) {
                    //未在线状态
                    node.setIcon(R.mipmap.ic_vehicle_stop);
                } else if (online.equals("online")) {
                    //在线状态
                    node.setIcon(R.mipmap.ic_vehicle_start);
                } else if (online.equals("parking")) {
                    //停止状态
                    node.setIcon(R.mipmap.ic_vehicle_p);
                }
            }
            nodes.add(node);
        }
...
//		else
//		node.setIcon(R.mipmap.ic_angle);
